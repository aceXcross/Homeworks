...
if __name__ == '__main__':
    teachers = [Teacher('Виктор', 'Робертович', 'Цой', 'Музыка'),
                Teacher('Павел', 'Валерьевич', 'Дуров', 'Информатика'),
                Teacher('Станислав', 'Валерьевич', 'Барецкий', 'Физкультура'),
                Teacher('Артемий', 'Андреевич', 'Лебедев', 'Изобразтельное искусство'),]
    classes = [Class_rooms('11 А', [teachers[0], teachers[1], teachers[2]]),
               Class_rooms('11 Б', [teachers[1], teachers[3], teachers[1]]),
    ...
    for i in classes:
        print('Учителя, преподающие в {} классе:'.format(i.class_room))
        for teacher in classes[0].teachersdict.values():
            print(teacher.get_full_name())
    
    for i in classes:
        print("Ученики в классе {}:".format(i.class_room))
        for st in students:
